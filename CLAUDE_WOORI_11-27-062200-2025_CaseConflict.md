...
- 작업: 제품 수정 버튼 (edit-2 아이콘), 제품 삭제 버튼 (trash-2 아이콘)
- 수정 버튼 클릭 시 제품명 변경 가능
...
### 문제: 바코드 스캔 시 "등록되지 않은 바코드"라고 나옴 (한글 입력 모드)
- **원인**: 바코드 스캐너가 한글 입력 모드로 입력되어 `P003-IN-40` → `ㅖ003-ㅑㅜ-40`으로 변환됨
- **해결책**:
  1. **HTML**: `<input>` 요소에 `lang="en"`, `inputmode="latin"` 속성 추가
  2. **CSS**: `.hidden-input`에 `ime-mode: disabled` 추가
  3. **JavaScript**: `compositionstart` 이벤트로 한글 입력 차단
  4. **근본 해결**: Windows 설정에서 바코드 스캐너를 **영문 입력 전용 장치**로 설정
- **디버깅 방법**:
  - F12 개발자 도구 → Console 탭에서 "스캔한 바코드" 로그 확인
  - 한글 자음/모음이 보이면 IME 문제임
  - 해결 후에도 문제가 있다면 Windows 한/영 전환키를 눌러 영문 모드로 전환
